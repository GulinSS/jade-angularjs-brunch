  ...
  module.exports = JadeAngularJsCompiler = (function() {
    ...
    JadeAngularJsCompiler.prototype.onCompile = function(compiled) {
      ...
      assets = _.filter(preResult, function(v) {
        return v.path.indexOf('assets') === -1;
      });
      this.writeStatic(assets);
      return this.writeModules(_.chain(preResult).difference(assets).each(this.attachModuleNameToTemplate).groupBy(function(v) {
        return v.module;
      }).map(function(v, k) {
        return {
          name: k,
      ...
      }).each(this.generateModuleFileName).value());
    };
